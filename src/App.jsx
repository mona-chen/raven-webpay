...
function handleCardNumberChange(event) {
  ...
  useEffect(() => {
    if (config?.redirect_url !== null) setCallbackUrl(`${config?.redirect_url}?trx_ref=${config?.trx_ref}&merchant_ref=${config?.merchant_ref}&status=${config?.status}`)
  }, [])
...
<div  className="payment_btn">
  <RavenButton disabled={ 
    (stage === "pin" && pinVal.length !== 6) || stage === "confirming-transaction" ||
    (stage === "main" && paymentMethod === "card" && (cvv.length < 2 || cardNumber.length < 8 || expiryDate.length < 3) ) 
  } 
  loading={(loading  || isLoading)  }
  label={stage === "confirming-transaction" ? "Confirming Transaction" : paymentMethod === "raven" && stage === "main" ? "I have sent the  money"  : paymentMethod !== "transfer" && stage !== "failed-transaction" ? `Pay NGN ${config?.amount}` :  stage === "failed-transaction" ? "Change payment method" : "I have sent the money"}
  color="green-light"
  className='pay_btn'
  onClick={() => {
    success ? navigate(callbackUrl) :
    paymentMethod === "raven" && stage === "main" ? 
    setStage('confirming-transaction') : stage === "main" && 
    paymentMethod === "card" ? initCardPayment() : stage === "main" && 
    paymentMethod === "transfer" ? setStage('confirming-transaction') : ''}}
  width="100%"
  />
</div>
